...
def load_single_label_boundary_data_oneside(dir_path, boundary_1, boundary_2, label_count, side):
    """
    ...
    if side == 0:
        ...
        x_train = np.append(x_train, single_x[1]).reshape(len(single_x[0]), 1, 28, 28, 1)
    y_tr = [label_count for k in range(int(len(single_x[0])))]
    y_tr = np.asarray(y_tr)
    return x_train, y_tr
